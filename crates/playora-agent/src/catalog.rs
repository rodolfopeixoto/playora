...
pub fn cmd_search(cfg: AgentConfig, term: &str) -> Result<()> {
    ...
    for it in fetch_all(&cfg)?.into_iter().filter(|i| {
        i.title.to_lowercase().contains(&term_l)
            || i.tags.iter().any(|t| t.to_lowercase().contains(&term_l))
    }) {
...
pub fn cmd_download(cfg: AgentConfig, id: &str) -> Result<()> {
    ...
    let item = items
        .into_iter()
        .find(|i| i.id == id)
        .ok_or_else(|| anyhow::anyhow!("catalog item not found: {id}"))?;
    let url = item
        .download_url
        .clone()
        .ok_or_else(|| anyhow::anyhow!("no download_url"))?;
    let install_path = item
        .install_path
        .clone()
        .unwrap_or_else(|| format!("/roms/.playora/cache/{}", item.id));
