...
pub fn cmd_launch(cfg: AgentConfig, system: &str, core: Option<&str>, rom: &str, command: &[String]) -> Result<()> {
    let session_id = SessionId::new();
    let started = Utc::now();
    let conn_open = crate::db::open(&crate::cfg::db_path());
    let game_name = std::path::Path::new(rom).file_stem().and_then(|s| s.to_str()).unwrap_or(rom).to_string();

    // Enqueue start event (best-effort)
    if let Ok(ref conn) = conn_open {
        let _ = crate::db::enqueue(conn, &Event {
