...
fn server_full_cycle() {
    ...
    assert!(wait_health(&client, &format!("{base}/health")), "server did not become healthy in time");
    assert_eq!(client.get(format!("{base}/health")).send().unwrap().text().unwrap_or_default(), "ok");
    ...
    let batch = SyncBatch {
        ...
        events: vec![Event {
            ...
            payload: EventPayload::GameSessionStarted(GameSessionStarted {
                ...
                rom_hash: None, core: None,
                started_at: chrono::Utc::now(),
            }),
        }],
    };
    let resp = client.post(format!("{base}/api/v1/events/batch")).json(&batch).send().unwrap();
    assert!(resp.status().is_success());
    let ack: SyncAck = resp.json().unwrap();
    assert_eq!(ack.accepted.len(), 1);
    assert!(ack.duplicates.is_empty());

    let resp2 = client.post(format!("{base}/api/v1/events/batch")).json(&batch).send().unwrap();
    let ack2: SyncAck = resp2.json().unwrap();
    assert_eq!(ack2.duplicates.len(), 1, "deduplication");

    let sources: Vec<playora_common::sources::RomSource> = client.get(format!("{base}/api/v1/sources")).send().unwrap().json().unwrap();
    assert!(!sources.is_empty());

    let systems: serde_json::Value = client.get(format!("{base}/api/v1/systems")).send().unwrap().json().unwrap();
    assert!(systems.as_array().map(|a| !a.is_empty()).unwrap_or(false));

    let mut map = BTreeMap::new();
    map.insert("netplay".to_string(), "enabled".to_string());
    let upd = client.put(format!("{base}/api/v1/devices/{}/features", dev)).json(&map).send().unwrap();
    assert!(upd.status().is_success());

    let manifest: FeatureManifest = client.get(format!("{base}/api/v1/devices/{}/manifest", dev)).send().unwrap().json().unwrap();
    assert_eq!(manifest.features.get("netplay"), Some(&FeatureStatus::Enabled));
