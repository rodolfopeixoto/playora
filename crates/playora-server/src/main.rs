...
async fn main() -> Result<()> {
    tracing_subscriber::fmt().with_env_filter(std::env::var("PLAYORA_LOG").unwrap_or_else(|_| "playora_server=info,tower_http=info".into())).init();
    let args = Args::parse();
    ...
    let app = Router::new()
        ...
        .route("/api/v1/hardware/test-result",  post(routes::hardware_test_result))
        ...
        .route("/api/v1/catalog/:id/download",  get(routes::catalog_download))
